...
export default function FAQAccordion() {
  ...
  return (
    <section id="faq" className="py-14 sm:py-20 lg:py-24 bg-navy-950 border-t border-slate-800/80 relative overflow-hidden">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        
        {/* Header */}
        <div className="text-center mb-10 sm:mb-16">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-md bg-slate-900 border border-slate-800 text-xs font-semibold text-slate-300 mb-3">
          ...
          <p className="text-slate-400 text-xs sm:text-base mt-3 max-w-xl mx-auto">
            ...
            return (
              <div
                ...
                className={`rounded-2xl border transition-all overflow-hidden ${
                  isOpen
                    ? 'bg-navy-900/90 border-blue-500/40 shadow-lg shadow-blue-500/5'
                    : 'bg-navy-900/40 border-slate-800 hover:border-slate-700'
                }`}
              >
                <button
                  onClick={() => toggleFAQ(index)}
                  className="w-full px-4 py-4 sm:px-6 sm:py-5 flex items-center justify-between gap-3 sm:gap-4 text-left font-heading font-semibold text-sm sm:text-lg text-white"
                ...
                {isOpen && (
                  <div className="px-4 pb-5 pt-1 sm:px-6 sm:pb-6 text-slate-300 text-xs sm:text-base leading-relaxed border-t border-slate-800/60 animate-in fade-in duration-200">
                    {faq.answer}
                  </div>
                )}
              </div>
